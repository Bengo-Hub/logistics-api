...
until MIGRATE_OUTPUT=$(POSTGRES_URL="$MIGRATE_URL" /usr/local/bin/logistics-migrate 2>&1) || [ $RETRY_COUNT -eq $MAX_RETRIES ]; do
...
echo "=========================================="
POSTGRES_URL="$MIGRATE_URL" /usr/local/bin/logistics-seed || echo "Seed completed with warnings (non-fatal)"
