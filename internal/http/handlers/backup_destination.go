...
// are encrypted at rest (via a SECRET_KEY-derived Cipher), masked in all
...
// logistics has no payment-gateway KeyProvider, so the Cipher is derived from the
// platform SECRET_KEY (see destination.NewSecretKeyCipher).
func NewBackupDestinationHandler(client *ent.Client, keys destination.Cipher, log *zap.Logger) *BackupDestinationHandler {
...
func (h *BackupDestinationHandler) GetTenant(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		respondJSON(w, http.StatusBadRequest, map[string]string{"error": "tenant required"})
		return
	}
	h.get(w, r, &tenantID)
}
