...
func (tx *Tx) init() {
	...
	tx.TaskStep = NewTaskStepClient(tx.config)
	tx.TelemetryPoint = NewTelemetryPointClient(tx.config)
	tx.TelemetryStream = NewTelemetryStreamClient(tx.config)
...
// applies a query, for example: BillingEvent.QueryXXX(), the query will be executed
