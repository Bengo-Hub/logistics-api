...
type UserRoleAssignment struct {
	...
	// User identifier (logistics user)
	...
	logistics_role_user_assignments *uuid.UUID
...
type UserRoleAssignmentEdges struct {
	...
	Role *LogisticsRole `json:"role,omitempty"`
...
func (e UserRoleAssignmentEdges) UserOrErr() (*User, error) {
	if e.User != nil {
		return e.User, nil
...
// was not loaded in eager-loading, or loaded but was not found.
func (e UserRoleAssignmentEdges) RoleOrErr() (*LogisticsRole, error) {
	if e.Role != nil {
		return e.Role, nil
	} else if e.loadedTypes[1] {
		return nil, &NotFoundError{label: logisticsrole.Label}
	}
...
func (*UserRoleAssignment) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case userroleassignment.ForeignKeys[0]: // logistics_role_user_assignments
	...
	for i := range columns {
		...
		case userroleassignment.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field logistics_role_user_assignments", values[i])
			} else if value.Valid {
				_m.logistics_role_user_assignments = new(uuid.UUID)
				*_m.logistics_role_user_assignments = *value.S.(*uuid.UUID)
			}
		default:
...
func (_m *UserRoleAssignment) QueryRole() *LogisticsRoleQuery {
