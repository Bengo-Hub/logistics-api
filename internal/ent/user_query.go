...
type UserQuery struct {
	...
	withFleetMemberships *FleetMemberQuery
	withTasks            *TaskQuery
...
// QueryFleetMemberships chains the current query on the "fleet_memberships" edge.
func (_q *UserQuery) QueryFleetMemberships() *FleetMemberQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withFleetMemberships: _q.withFleetMemberships.Clone(),
...
// WithFleetMemberships tells the query-builder to eager-load the nodes that are connected to
// the "fleet_memberships" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithFleetMemberships(opts ...func(*FleetMemberQuery)) *UserQuery {
	query := (&FleetMemberClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withFleetMemberships = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withFleetMemberships; query != nil {
		if err := _q.loadFleetMemberships(ctx, query, nodes,
			func(n *User) { n.Edges.FleetMemberships = []*FleetMember{} },
			func(n *User, e *FleetMember) { n.Edges.FleetMemberships = append(n.Edges.FleetMemberships, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadFleetMemberships(ctx context.Context, query *FleetMemberQuery, nodes []*User, init func(*User), assign func(*User, *FleetMember)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(fleetmember.FieldUserID)
	}
	query.Where(predicate.FleetMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.FleetMembershipsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
