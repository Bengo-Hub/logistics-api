...
type UserEdges struct {
	...
	// FleetMemberships holds the value of the fleet_memberships edge.
	FleetMemberships []*FleetMember `json:"fleet_memberships,omitempty"`
...
// FleetMembershipsOrErr returns the FleetMemberships value or an error if the edge
// was not loaded in eager-loading.
func (e UserEdges) FleetMembershipsOrErr() ([]*FleetMember, error) {
	if e.loadedTypes[1] {
		return e.FleetMemberships, nil
	}
	return nil, &NotLoadedError{edge: "fleet_memberships"}
}
...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.FieldMetadata:
			values[i] = new([]byte)
		case user.FieldEmail, user.FieldSyncStatus, user.FieldFullName, user.FieldPhone, user.FieldStatus, user.FieldRole:
			values[i] = new(sql.NullString)
		case user.FieldSyncAt, user.FieldCreatedAt, user.FieldUpdatedAt:
...
// QueryFleetMemberships queries the "fleet_memberships" edge of the User entity.
func (_m *User) QueryFleetMemberships() *FleetMemberQuery {
	return NewUserClient(_m.config).QueryFleetMemberships(_m)
}
