...
// HasFleetMemberships applies the HasEdge predicate on the "fleet_memberships" edge.
func HasFleetMemberships() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasFleetMembershipsWith applies the HasEdge predicate on the "fleet_memberships" edge with a given conditions (other predicates).
func HasFleetMembershipsWith(preds ...predicate.FleetMember) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newFleetMembershipsStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
