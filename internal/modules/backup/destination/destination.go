...
// for the logistics backup module. The local PVC copy produced by the backup
...
// rest with AES-256-GCM using a Cipher (see cipher.go — a SECRET_KEY-derived key),
// so secret backend params (access keys, tokens, passwords) are never stored or
// logged in plaintext.
//
...
// services without one (like logistics) pass a SECRET_KEY-derived static key
// (see cipher.go).
...
type Destination struct {
	...
	// under (e.g. "logistics/backups", an S3 key prefix, or a Drive folder path).
