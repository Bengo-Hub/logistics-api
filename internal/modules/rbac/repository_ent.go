...
func (r *EntRepository) CreateRole(ctx context.Context, tenantID uuid.UUID, role *LogisticsRole) error {
	if role == nil {
		return errors.New("role cannot be nil")
	}

	builder := r.client.LogisticsRole.Create().
		SetID(role.ID).
...
func (r *EntRepository) GetRole(ctx context.Context, tenantID uuid.UUID, roleID uuid.UUID) (*LogisticsRole, error) {
	entRole, err := r.client.LogisticsRole.Query().
		Where(
			logisticsrole.ID(roleID),
			logisticsrole.TenantID(tenantID),
		).
		Only(ctx)
	if err != nil {
...
func (r *EntRepository) GetRoleByCode(ctx context.Context, tenantID uuid.UUID, roleCode string) (*LogisticsRole, error) {
	entRole, err := r.client.LogisticsRole.Query().
		Where(
			logisticsrole.RoleCode(roleCode),
			logisticsrole.TenantID(tenantID),
		).
		Only(ctx)
	if err != nil {
...
func (r *EntRepository) ListRoles(ctx context.Context, tenantID uuid.UUID) ([]*LogisticsRole, error) {
	entRoles, err := r.client.LogisticsRole.Query().
	...
	roles := make([]*LogisticsRole, len(entRoles))
...
func (r *EntRepository) CreatePermission(ctx context.Context, permission *LogisticsPermission) error {
	if permission == nil {
		return errors.New("permission cannot be nil")
	}

	builder := r.client.LogisticsPermission.Create().
		SetID(permission.ID).
...
func (r *EntRepository) GetPermission(ctx context.Context, permissionID uuid.UUID) (*LogisticsPermission, error) {
	entPerm, err := r.client.LogisticsPermission.Get(ctx, permissionID)
	if err != nil {
		if ent.IsNotFound(err) {
...
func (r *EntRepository) GetPermissionByCode(ctx context.Context, permissionCode string) (*LogisticsPermission, error) {
	entPerm, err := r.client.LogisticsPermission.Query().
		Where(logisticspermission.PermissionCode(permissionCode)).
		Only(ctx)
	if err != nil {
		if ent.IsNotFound(err) {
			return nil, fmt.Errorf("permission not found: %w", err)
...
func (r *EntRepository) ListPermissions(ctx context.Context, filters PermissionFilters) ([]*LogisticsPermission, error) {
	query := r.client.LogisticsPermission.Query()

	if filters.Module != nil {
		query = query.Where(logisticspermission.Module(*filters.Module))
	}
	if filters.Action != nil {
		query = query.Where(logisticspermission.Action(*filters.Action))
	}
	...
	permissions := make([]*LogisticsPermission, len(entPerms))
...
func (r *EntRepository) GetRolePermissions(ctx context.Context, roleID uuid.UUID) ([]*LogisticsPermission, error) {
	entPerms, err := r.client.LogisticsRole.Query().
		Where(logisticsrole.ID(roleID)).
		QueryPermissions().
		All(ctx)
	if err != nil {
...
func (r *EntRepository) GetUserRoles(ctx context.Context, tenantID uuid.UUID, userID uuid.UUID) ([]*LogisticsRole, error) {
...
func (r *EntRepository) GetUserPermissions(ctx context.Context, tenantID uuid.UUID, userID uuid.UUID) ([]*LogisticsPermission, error) {
	...
	permissionMap := make(map[uuid.UUID]*LogisticsPermission)
	...
	permissions := make([]*LogisticsPermission, 0, len(permissionMap))
...
func mapEntRole(entRole *ent.LogisticsRole) *LogisticsRole {
	role := &LogisticsRole{
		ID:           entRole.ID,
...
func mapEntPermission(entPerm *ent.LogisticsPermission) *LogisticsPermission {
